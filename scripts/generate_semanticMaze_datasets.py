...
from d4rl.pointmaze.semantic_maze_layouts import SEMANTIC_MAZE_LAYOUTS, SKILL2ID, semantic_layout2str, xy2id, xy2coord
import numpy as np
import pickle
import gzip
import h5py
...
def get_skill(state, last_room_id, layout, skill_boundaries):
    ...
    if curr_room_id != last_room_id and curr_room_id == -1:
        ...
        assert last_room_id in connected_rooms      # need to have come from one room adjacent to door
        skill = str(connected_rooms[0])+'-'+str(connected_rooms[1]) if connected_rooms[0] == last_room_id \
...
def main():
    ...
    parser.add_argument('--layout_id', type=int, default=int(-1), help='ID of layout used')
